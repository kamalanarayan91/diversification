...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    //  #near/n and #window/n.
    int distance = -1;
    int slashIndex = operatorNameLowerCase.indexOf('/');
    if(slashIndex!=-1)
    {
        //near operator
        distance = Integer.parseInt(operatorNameLowerCase.substring(slashIndex+1));
        operatorNameLowerCase = operatorNameLowerCase.substring(0,slashIndex);
	...
	break;
        ...
        case "#near":
        operator = new QryIopNear(distance);
        break;

      case "#window":
        operator= new QryIopWindow(distance);
        break;
    ...
    if(operator instanceof QrySopWand || operator instanceof QrySopWsum )
    {
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
      ...
      {                    // Term
            ...
            if(isNumber(p.getPopped()))
            {
              if(previousWeight==false)
              {

                if (queryTree instanceof QrySopWand)
                {
                  ((QrySopWand) queryTree).addWeight(Double.parseDouble(p.getPopped()));
                  previousWeight=true;
                }
                else if (queryTree instanceof QrySopWsum)
                {
                  ((QrySopWsum) queryTree).addWeight(Double.parseDouble(p.getPopped()));
                  previousWeight=true;
                }
                else
                {
            ...
            /*if (Character.isDigit(p.getPopped().charAt(0)) || p.getPopped().charAt(0) == '.')
            {

                double number =  Double.parseDouble(p.getPopped());
                ...
                if(number<1)
                {
                    ...
                    {
                        System.out.println("Error in weights- Query operator is wrong");
                    }
      ...
      if (qargs != null)
      {
        ...
        {
          if(queryTree instanceof QrySopWand) {
            ((QrySopWand) queryTree).removeWeight();
          }
          else if(queryTree instanceof QrySopWsum){
            ((QrySopWsum) queryTree).removeWeight();
          }
        }
